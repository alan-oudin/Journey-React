...
const gridPagination = require('./grid-pagination-4505d7b6.js');
...
function getAllKeysIn(object) {
  return isEqual.baseGetAllKeys(object, keysIn, getSymbolsIn$1);
}
...
const GridRadio = ({ checked, row, rowIndex, totalRows, onClick }) => {
    let input;
    return (index.h("div", { class: `grid-radio ${checked ? 'checked' : ''}`, onClick: _ => input.click() },
        index.h("input", { type: "radio", name: "selection", "aria-setsize": totalRows, "aria-posinset": rowIndex, tabindex: "-1", checked: checked, "aria-checked": `${checked}`, ref: (el) => (input = el), onClick: _ => onClick(row) })));
};
...
const Grid = class {
    ...
    componentWillLoad() {
        this.inheritedAttributes = Object.assign(Object.assign({}, helpers.inheritAriaAttributes(this.el)), helpers.inheritAttributes(this.el, GRID_INHERITED_ATTRS));
    }
    async setAriaAttribute(attr, value) {
        helpers.setOrRemoveAttribute(this.nativeTable, attr, value);
    }
    /**
    ...
    renderSelectionColumn(row, rowIndex) {
        ...
        switch (this.selectionConfig) {
            case 'none':
                return;
            case 'single':
                return index.h("td", { "aria-colindex": this.atLeastOneColumnHidden() ? 1 : null, tabIndex: ((_a = this.cursorPosition) === null || _a === void 0 ? void 0 : _a.col) === 0 && rowIndex + 1 === ((_b = this.cursorPosition) === null || _b === void 0 ? void 0 : _b.row) ? 0 : -1 }, index.h(GridRadio, { checked: row.selected, row: row, rowIndex: rowIndex + 1, totalRows: this.rows.length, onClick: (row) => {
                        if (!row.selected) {
                            this.onRowSelection(row);
                        }
                    } }));
            case 'multiple':
                return index.h("td", { "aria-colindex": this.atLeastOneColumnHidden() ? 1 : null, tabIndex: ((_c = this.cursorPosition) === null || _c === void 0 ? void 0 : _c.col) === 0 && rowIndex + 1 === ((_d = this.cursorPosition) === null || _d === void 0 ? void 0 : _d.row) ? 0 : -1 }, index.h("wcs-checkbox", { tabIndex: -1, part: "row-checkbox", checked: row.selected, onWcsChange: this.onRowSelection.bind(this, row) }));
        }
    }
    ...
    render() {
        var _a, _b, _c, _d;
        return (index.h(index.Host, { key: '98d13e7497ec9d9ecd3e149b3f6986e16bc5cc2a' }, index.h("table", Object.assign({ key: 'a44825cf71ff284a5204e9d922c0eba2c2ca23f4', role: "grid", ref: (el) => (this.nativeTable = el), "aria-rowcount": !this.loading && ((_a = this.rows) === null || _a === void 0 ? void 0 : _a.length), "aria-colcount": !this.loading && this.totalDisplayedColumnCount() }, this.inheritedAttributes), index.h("thead", { key: '336578d350ae8082a19fa6798af35f2fdd015ff9' }, index.h("tr", { key: 'fdb38402ea5c58dd77dad7df2ef97d54c521123c', "aria-rowindex": "1" }, this.selectionConfig === 'none' ? ''
            : index.h("th", { class: "wcs-grid-selection-column" }, this.selectionConfig === 'single' ? '' :
                index.h("wcs-checkbox", { tabIndex: ((_b = this.cursorPosition) === null || _b === void 0 ? void 0 : _b.col) === 0
                        && ((_c = this.cursorPosition) === null || _c === void 0 ? void 0 : _c.row) === 0 ? 0 : -1, part: "all-rows-checkbox", checked: this.allRowsAreSelected(), onWcsChange: this.selectAllRows.bind(this) })), index.h("slot", { key: '2d68b72fac0251dab9f519cf84c25c96262c193b', name: "grid-column" }))), index.h("tbody", { key: '4156e796ac5a81170b5dba011159f8e6bb7a7e60' }, this.loading
            ? index.h("tr", { "aria-busy": "true" }, index.h("td", { colSpan: this.totalColumnCount(), class: "loading" }, index.h("wcs-spinner", null)))
            : (_d = this.rows) === null || _d === void 0 ? void 0 : _d.filter(row => this.serverMode || !this.paginationEl || row.page === this.paginationEl.currentPage).map((row, index) => this.renderRow(row, index)))), index.h("slot", { key: 'c683eb883707ffbd1fed22d86ececb595f70c2c0', name: "grid-pagination" })));
    }
    /**
    ...
    renderRow(row, rowIndex) {
        ...
        return index.h("tr", { class: row.selected ? 'selected' : '', part: (_a = this.rowCssPartsWithoutEmptyList(row)) === null || _a === void 0 ? void 0 : _a.join(' '), "aria-selected": row.selected ? 'true' : null, "aria-rowindex": rowIndex + 2 }, this.renderSelectionColumn(row, rowIndex), (_b = row.cells) === null || _b === void 0 ? void 0 :
            _b.map((cell, cellIndex) => {
                ...
                return cell.column.customCells
                    ? (index.h("td", { tabIndex: cursorIsOnCell ? 0 : -1, "aria-colindex": this.atLeastOneColumnHidden() ? nonHiddenColumnIndex : null }, index.h("slot", { name: cell.column.id + '-' + get(row.data, this.rowIdPath) })))
